...
sequenceDiagram
    ...
    participant UJ as upload_job
    participant UW as UploadJobWorker
    participant CSV as CsvParserService
    participant EJ as email_job
    participant OB as outbox_event
    participant OP as OutboxPublisherWorker
    participant RQ as Redis Queue
    participant ES as EmailSenderWorker
    participant EP as EmailProvider

    C->>API: POST /api/mail/upload (CSV)
    API->>US: createUploadJob(file)
    US->>FS: store(file)
    US->>UJ: save(PENDING)
    API-->>C: uploadJobId

    UW->>UJ: poll PENDING
    UW->>CSV: parseUpload(uploadJob)
    CSV->>EJ: saveAll(batch email jobs)
    CSV->>OB: saveAll(batch outbox events)
    CSV->>UJ: update processedRows

    OP->>OB: poll NEW events
    OP->>RQ: publish(emailJobId)
    OP->>OB: mark SENT

    loop worker threads
        ES->>RQ: consume jobId
        ES->>EJ: load PENDING job
        ES->>EP: sendEmail(...)
        alt success
            ES->>EJ: mark SUCCESS
        else retryable failure
            ES->>RQ: publish again
            ES->>EJ: increment retryCount
